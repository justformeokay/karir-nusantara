...
export async function getWishlist(params: {
  ...
  const response = await api.get<WishlistListResponse>(endpoint);
  ...
  return {
    items: (response.data.data || []).map(transformWishlistItem),
 ...
 */
export async function saveToWishlist(jobId: number): Promise<SavedJob> {
 ...
 */
export async function removeFromWishlist(jobId: number): Promise<void> {
 ...
 */
export async function checkIsJobSaved(jobId: number): Promise<boolean> {
  try {
